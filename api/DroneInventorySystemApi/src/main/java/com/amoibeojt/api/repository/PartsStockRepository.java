...
public interface PartsStockRepository extends JpaRepository<PartsStock, Integer> {
	...
	/**
	 * 検索条件に一致する部品在庫情報を取得する。
	 * JOIN句を使用してカテゴリ名・拠点名を含むDTOにマッピングする。
	 *
	 * @param centerIds 拠点IDリスト（null許容）
	 * @param categoryIds カテゴリIDリスト（null許容）
	 * @param stockIds 在庫IDリスト（null許容）
	 * @param namePattern 名称パターン（null許容）
	 * @param amountMin 最小在庫数（null許容）
	 * @param amountMax 最大在庫数（null許容）
	 * @return DTO形式の部品在庫情報リスト
	 */
	@Query("""
